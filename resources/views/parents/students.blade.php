...
@section('title', "Parent's Students")

@section('left_header')
	{{ $user->name }}'s Students
@endsection
...
@section('page_content')
	<div class="col-lg-12">
			...
			<form method="get">
				<div class="card-body row align-items-center">
					...
					<div class="col-4">
						...
						<a class="btn btn-danger px-4" href="{{ route('parents.students', $user->id) }}" style="position: relative; top: 7px;">
							Reset
						</a>
					</div>
				</div>
							...
							@foreach ($student_result as $key => $student)
								<tr>
									...
									<td class="d-flex" style="gap: 7px">
										<form method="post" action="{{ route('parents.assignStudent', [$user->id, $student->id]) }}">
											@csrf
		...
		<div class="card card-light">
			<div class="card-header">
				<h3 class="card-title">Parent's Students</h3>
			</div>
					...
					<tbody>
						@foreach ($my_students as $key => $student)
							<tr>
								...
								<td class="d-flex" style="gap: 7px">
									<form method="post" action="{{ route('parents.removeStudent', $student->id) }}">
										@method('delete')
										@csrf
